...
function updateFigure() {
  ...
  let leftWristPos = new THREE.Vector3(
    bodyFrame[15].x,
    bodyFrame[15].y,
    bodyFrame[15].z
  );
  ...
  setBoneDirection(
    bonesMap["ID56"], // e.g. "LeftArm" bone
    leftShoulderPos,
    leftElbowPos,
    new THREE.Vector3(0, -1, 0) // example restPoseDir
  );

  setBoneDirection(
    bonesMap["ID57"], // e.g. "LeftForeArm" bone
    leftElbowPos,
    leftWristPos,
    new THREE.Vector3(0, -1, 0) // example restPoseDir
  );
...
function setBoneDirection(bone, parentPos, childPos, restPoseDir) {
  // Compute the current direction vector from parentPos to childPos
  const currentDir = new THREE.Vector3()
    .subVectors(childPos, parentPos)
    .normalize();

  const axis = new THREE.Vector3()
    .crossVectors(restPoseDir, currentDir)
    .normalize();
  const angle = Math.acos(restPoseDir.dot(currentDir));
